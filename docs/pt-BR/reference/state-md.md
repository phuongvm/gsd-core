...
`normalizeStateStatus()` em `get-ship-done/bin/lib/state-document.cjs` mapeia o texto bruto do corpo para estes valores canônicos:
...
O corpo (tudo após o `---` de fechamento) segue o template em `get-ship-done/templates/state.md`. As seções padrão são:
